...
func getSingleGameHeader() []string {
	return []string{
		...
		// ML Pipeline Features (CS2 Synergy & Role-Gap Engine)
		"Space Creation Index", "Uncontested Advance", "Crosshair Displacement", "Trade Window Median",
		"Utility Efficiency Score", "Expected Flash Blindness", "Blind To Kill Conversion",
		"Smoke Effectiveness", "Smokes Thrown", "Molotov Delay", "Molotovs Thrown",
		"Anchor Hold Time", "Anchor Rounds",
		"Lurk Rounds", "Lurk Kills", "Lurk Plants", "Flank Success Rate",
		"Information Denial Rounds", "Clock Efficiency",
		"AWP Buy Rounds", "AWP Usage Rate", "AWP Opening Duel Attempts", "AWP Opening Duel Wins", "AWP Opening Duel Win Rate",
		"Resource Score", "Damage Per Dollar", "Economy Efficiency",
		"Crossfire Partner Distance", "Entry Spacing SD",
		"Entry Attempt Rate", "First Contact Rate", "First Contact Rounds",
		"Mid Round Utility T", "Mid Round Utility CT", "Mid Round Utility Usage",
	}
}
...
func getSingleGameRow(p *model.PlayerStats) []string {
	return []string{
		...
		// ML Pipeline Features (CS2 Synergy & Role-Gap Engine)
		formatFloat(p.SpaceCreationIndex),
		formatFloat(p.UncontestedAdvance),
		strconv.Itoa(p.CrosshairDisplacement),
		formatFloat(p.TradeWindowMedian),
		formatFloat(p.UtilityEfficiencyScore),
		formatFloat(p.ExpectedFlashBlindness),
		formatFloat(p.BlindToKillConversion),
		formatFloat(p.SmokeEffectiveness),
		strconv.Itoa(p.SmokesThrown),
		formatFloat(p.MolotovDelay),
		strconv.Itoa(p.MolotovsThrown),
		formatFloat(p.AnchorHoldTime),
		strconv.Itoa(p.AnchorRounds),
		strconv.Itoa(p.LurkRounds),
		strconv.Itoa(p.LurkKills),
		strconv.Itoa(p.LurkPlants),
		formatFloat(p.FlankSuccessRate),
		strconv.Itoa(p.InformationDenialRounds),
		formatFloat(p.ClockEfficiency),
		strconv.Itoa(p.AWPBuyRounds),
		formatFloat(p.AWPUsageRate),
		strconv.Itoa(p.AWPOpeningDuelAttempts),
		strconv.Itoa(p.AWPOpeningDuelWins),
		formatFloat(p.AWPOpeningDuelWinRate),
		formatFloat(p.ResourceScore),
		formatFloat(p.DamagePerDollar),
		formatFloat(p.EconomyEfficiency),
		formatFloat(p.CrossfirePartnerDistance),
		formatFloat(p.EntrySpacingSD),
		formatFloat(p.EntryAttemptRate),
		formatFloat(p.FirstContactRate),
		strconv.Itoa(p.FirstContactRounds),
		strconv.Itoa(p.MidRoundUtilityT),
		strconv.Itoa(p.MidRoundUtilityCT),
		formatFloat(p.MidRoundUtilityUsage),
	}
}
